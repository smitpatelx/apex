...
function trim_between($value) {
    return trim(preg_replace('/\s+/', '', $value));
}

function clean($string) {
    $string = str_replace(' ', '', $string); // Replaces all spaces with nothing.
...
function convert_phone($phonenumber) {
    ...
    && ($dial_sequence < MAX_DIAL_SEQUENCE && $dial_sequence > MIN_DIAL_SEQUENCE)) {
        ...
        return "(".$areacode.")".$exchange." ".$dial_sequence;
    } else {
        //echo "false";
        return "(658) 698 5698";
    }
...
function random_obj($value) {
    
    //$rand = mt_rand(count($value));

    return array_rand($value);
}
?>